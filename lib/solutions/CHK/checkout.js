...
module.exports = function(skus) {
  try {
    ...
    for (let item in inventory) {
      const { price, offers } = inventory[item]
      if (inventory[item].count && inventory[item].count > 0) {
        if (offers) {
          offers.forEach(offer => {
            switch (offer.type) {
              case 'group': {
                const remainer = inventory[item].count % offer.count
                total +=
                  ((inventory[item].count - remainer) / offer.count) *
                  offer.price
                inventory[item].count = remainer
                break
              }
              case 'freeItem': {
                if (inventory[item].count >= offer.count) {
                  const removal = Math.floor(
                    inventory[item].count / offer.count
                  )
        ...
        total += inventory[item].count * price
      }
    }
